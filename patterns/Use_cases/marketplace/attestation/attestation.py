...
if __name__ == "__main__":
    ...
    attr1 = int.to_bytes(1337, 64, "big") # attr id for Birthday
    vc1= int.to_bytes(19810902, 64, "big")
    ...
    resultHash = hashlib.sha256(b"".join([attr1[-32:], vc1[-32:]])).digest()
    resultHash += resultHash

    sig2 = signKey.sign(resultHash)
    ...
    outputs = [
        ...
        write_signature_for_zokrates_cli(pk, sig1, msg),
        " ".join([str(i) for i in struct.unpack(">16I", dapp_id)][-8:]),
        " ".join([str(i) for i in struct.unpack(">16I", attr1)][-8:]),
        " ".join([str(i) for i in struct.unpack(">16I", vc1)][-8:]),        #"19890519",
        write_signature_for_zokrates_cli(verifyKey, sig2, resultHash),
    ]
