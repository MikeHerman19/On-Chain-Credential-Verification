...
if __name__ == "__main__":
    ...
    attr11 = int.to_bytes(9, 64, "big") # attr id for zip code
    vc_value11 = int.to_bytes(45054, 64, "big")

    resultHash11 = hashlib.sha256(b"".join([attr11[-32:], vc_value11[-32:]])).digest()
    resultHash11 += resultHash11

    attr12 = int.to_bytes(9, 64, "big") # attr id for zip code
    vc_value12 = int.to_bytes(45054, 64, "big")

    resultHash12 = hashlib.sha256(b"".join([attr12[-32:], vc_value12[-32:]])).digest()
    resultHash12 += resultHash12
    ...
    sig11 = signKey.sign(resultHash11)
    sig12 = signKey.sign(resultHash12)
    ...
    outputs = [
        ...
        " ".join([str(i) for i in struct.unpack(">16I", attr11)][-8:]),
        " ".join([str(i) for i in struct.unpack(">16I", vc_value11)][-8:]),
        write_signature_for_zokrates_cli(verifyKey, sig11, resultHash11),
        " ".join([str(i) for i in struct.unpack(">16I", attr12)][-8:]),
        " ".join([str(i) for i in struct.unpack(">16I", vc_value12)][-8:]),
        write_signature_for_zokrates_cli(verifyKey, sig12, resultHash12),
    ]
